...
def compute_hourly_statistics(gate_id: int, hour_timestamp: datetime = None):
    try:
        ...
        detections_pipeline = [{"$match": {"gate_id": gate_id, "timestamp": {"$gte": hour_bucket, "$lt": next_hour}}}, {"$group": {"_id": "$agent_type", "count": {"$sum": 1}, "avg_confidence": {"$avg": "$detection_data.confidence"}}}]
        detections_stats = list(agent_detections_collection.aggregate(detections_pipeline))
        decisions_pipeline = [{"$match": {"gate_id": gate_id, "created_at": {"$gte": hour_bucket, "$lt": next_hour}}}, {"$group": {"_id": "$final_decision", "count": {"$sum": 1}}}]
        decisions_stats = list(decision_events_collection.aggregate(decisions_pipeline))
        stats_doc = {"gate_id": gate_id, "hour_bucket": hour_bucket, "detections": {"total": sum(s["count"] for s in detections_stats), "by_agent": {s["_id"]: s["count"] for s in detections_stats}, "avg_confidence": {s["_id"]: round(s["avg_confidence"], 3) for s in detections_stats}}, "decisions": {"total": sum(s["count"] for s in decisions_stats), "by_type": {s["_id"]: s["count"] for s in decisions_stats}}, "computed_at": datetime.now(timezone.utc), "version": 1}
        statistics_hourly_collection.update_one({"gate_id": gate_id, "hour_bucket": hour_bucket}, {"$set": stats_doc}, upsert=True)
        return stats_doc
    except Exception as e:
