...
# SCRIPT_DIR is where this script resides; project root is assumed to be
# the parent directory of SCRIPT_DIR so the script may live in a subfolder
# such as env_manager/ instead of the repository root.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# If we are running from a subdirectory, treat its parent as project root
